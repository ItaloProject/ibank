...
export function InvestimentosApp({ section }: { section: InvestimentosSection }) {
  ...
  const pageDescription =
    section === "contas"
      ? "Renda fixa, Tesouro e caixinhas"
      : section === "acoes"
        ? "Carteira de ações, FIIs e ETFs"
        : "Poupança, renda fixa e ações";
  ...
  return (
    <PageShell>
        ...
        description={
          <span className="inline-flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground hover:text-foreground hover:bg-muted transition-colors touch-manipulation shrink-0"
              title="Rentabilidade estimada"
              aria-label="Rentabilidade estimada"
            >
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          <Dialog open={accOpen} onOpenChange={setAccOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" size="sm" className="min-h-10 flex-1 sm:flex-none">
          ...
          <Dialog open={invOpen} onOpenChange={setInvOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" size="sm" disabled={accounts.length === 0} className="min-h-10 flex-1 sm:flex-none">
          ...
          {(section === "hub" || section === "acoes") && (
          <Dialog open={stockOpen} onOpenChange={setStockOpen}>
            <DialogTrigger asChild>
              <Button size="sm" className="min-h-10 flex-1 sm:flex-none">
      ...
      {section === "hub" ? (
        <div className="space-y-3">
          {accounts.length === 0 && stockTrades.length === 0 ? (
            ...
            <>
              ...
              >
                <span className="h-11 w-11 rounded-full bg-emerald-500/15 flex items-center justify-center shrink-0">
                  <Landmark className="h-5 w-5 text-emerald-500" />
                </span>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold">Contas</p>
                  <p className="text-xs text-muted-foreground">
                    {accountBalances.length} conta{accountBalances.length !== 1 ? "s" : ""} · renda fixa e Tesouro
                  </p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-sm font-extrabold tabular-nums">{formatCurrency(totalFixedIncome)}</p>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {stockPositions.length} ativo{stockPositions.length !== 1 ? "s" : ""} · bolsa
                  </p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-sm font-extrabold tabular-nums">{formatCurrency(totalStocks)}</p>
                  <ChevronRight className="h-4 w-4 text-muted-foreground ml-auto mt-0.5" />
                </div>
              </Link>
            </>
        ...
        <div className="space-y-4">
          {section === "contas" && accountBalances.length === 0 ? (
            <EmptyCaixinha
              icon={Landmark} iconColor="text-emerald-600"
              label="Nenhuma conta ainda"
              desc="Cadastre contas de renda fixa, Tesouro Direto ou caixinhas Turbo."
            ...
            <>
              {section === "contas" && accountBalances.length > 1 && (
                <SubPills
                  items={accountBalances.map((x) => ({ id: x.account.id, label: x.account.name }))}
                  active={activeTab}
